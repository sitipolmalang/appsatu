defmodule AppsatuWeb.UserAuthTest do
  use AppsatuWeb.ConnCase, async: true

  alias Phoenix.LiveView
  alias Appsatu.Accounts
  alias Appsatu.Accounts.Scope
  alias AppsatuWeb.UserAuth

  import Appsatu.AccountsFixtures
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, AppsatuWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      AppsatuWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        AppsatuWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
